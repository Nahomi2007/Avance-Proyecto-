...
public class GestionFinanciera {
    ...
    /**Recorre el gestor de órdenes buscando una orden activa que pertenezc al cliente ingresado y que se encuentre FINALIZADA por el área técnica.
     */
     ...
     */
    public double calcularSubtotalMateriales(OrdenServicio orden, GestionInventario gestionInventario) {
        double subtotalMateriales = 0.0;
        if (orden.getMateriales() != null) {
            for (String material : orden.getMateriales()) {
                try {
                    /**En la orden los materiales se guardan por código*/
                    int codigo = Integer.parseInt(material.trim());
                    int idx = gestionInventario.buscarCodigo(codigo);
                    if (idx != -1) {
                        subtotalMateriales += gestionInventario.getValor(idx).getPrecio();
                    } else {
                        subtotalMateriales += 15.00; /**código no encontrado en inventario*/
                    }
                } catch (Exception ex) {
                    subtotalMateriales += 15.00; /**material sin código válido*/
                }
            }
        }
    ...
    /**Actualización del metodo de pago de una factura valida*/
